...
import { ArrowLeft, ArrowRight, Zap, Shield, Hexagon, ArrowRightCircle } from 'lucide-react';
            ...
            <section className="relative h-[500px] print:h-auto print:py-8 print:bg-transparent print:h-auto print:py-8 flex items-center justify-center overflow-hidden bg-[#1a237e] print:bg-transparent">
                ...
                <div className="relative z-10 container mx-auto px-6 text-center mt-20">
                    ...
                    <h1 className="text-5xl md:text-6xl font-extrabold mb-8 leading-tight tracking-tight flex flex-col gap-2">
                        <span className="text-white drop-shadow-sm print:text-black print:drop-shadow-none">시스템</span>
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-400 via-emerald-400 to-cyan-400 print:text-black print:-webkit-text-fill-color-initial">
                            보안
                        </span>
                    </h1>
                    <p className="text-blue-100/80 print:text-black text-lg md:text-xl max-w-2xl mx-auto font-light leading-relaxed break-keep print:text-black">
                        내부 시스템 접속 경로를 일원화하여<br className="hidden md:block" />
                        가장 강력하고 안전한 통합 통제를 제공하는 솔루션
                    </p>
                </div>
            </section>
                    ...
                    <div className="flex flex-col md:flex-row gap-12 lg:gap-20">
                        ...
                        <div className="w-full md:w-3/4 print:w-full">
                            <div className="space-y-16 print:space-y-0 print:grid print:grid-cols-1 print:gap-8">
                                {solutions.map((item) => (
                                    <div key={item.id} className="group relative bg-white border border-slate-200 rounded-[2.5rem] p-10 md:p-12 print:p-6 print:shadow-none hover:border-teal-500 hover:shadow-2xl hover:shadow-teal-500/10 transition-all duration-300 overflow-hidden">

                                        {/* Background Decor */}
                                        <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50/50 rounded-full blur-3xl group-hover:bg-teal-50/50 transition-colors -mr-16 -mt-16"></div>

                                        <div className="relative z-10">
                                            {/* Header */}
                                            <div className="flex flex-col md:flex-row md:items-start gap-6 mb-8">
                                                <div className="w-16 h-16 rounded-2xl bg-teal-50 border border-teal-100 flex items-center justify-center text-teal-600 shrink-0 shadow-sm group-hover:scale-110 transition-transform duration-300">
                                                ...
                                                <div className="flex-grow">
                                                    ...
                                                    <p className="text-teal-500 font-medium text-lg mb-4">{item.subtitle}</p>
                                                    ...
                                                    {/* @ts-ignore */}
                                                    {/*
                                                    <Link href={item.externalLink || `/solutions/system/${item.id}`} className="inline-flex items-center mt-4 text-sm font-bold text-teal-600 hover:text-teal-800 transition-colors gap-1 print:hidden"
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                                ...
                                                {item.features.map((feature, idx) => (
                                                    <div key={idx} className="flex items-start gap-3 p-4 rounded-2xl bg-slate-50/80 hover:bg-teal-50/50 transition-colors border border-transparent hover:border-teal-100">
                                            ...
                                            <div className="flex flex-wrap items-center gap-3">
                                                <span className="text-sm font-bold text-slate-400 uppercase tracking-widest mr-2 flex items-center gap-2">
                                                    <Shield className="w-4 h-4" /> Specs
                                                </span>
                                                {item.specs.map((spec, i) => (
                                                    <span key={i} className="px-4 py-2 rounded-lg bg-white border border-slate-200 text-slate-600 text-sm font-bold shadow-sm flex items-center gap-2 group-hover:border-teal-200 group-hover:text-teal-600 transition-all">
                                                        <Hexagon className="w-3 h-3 text-slate-400 group-hover:text-teal-400" />
                                                        {spec}
                                                    </span>
                                                ))}
                                            </div>
